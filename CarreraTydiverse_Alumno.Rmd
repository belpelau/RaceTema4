...
```{r}
#Ponemos la función as_tibble y dentro el nombre de nuestro dataframe. A esta operación le llamamos cran.
cran <- as_tibble(mydf)
cran
#Para borrar el dataframe original usamos rm.
rm(mydf)
```
...
```{r}
#Ponemos el nombre de nuestra variable para ver la visualización.
cran
#Usamos sobre cran la función glimpse.
glimpse(cran)
#El resultado muestra el número de filas y columnas en el conjunto de datos, seguido de una lista de las columnas con sus nombres, tipos de datos y algunos valores de muestra. Esto proporciona una vista rápida de la estructura de los datos y es útil especialmente cuando se trabaja con conjuntos de datos grandes.
```
# dplyr
...
```{r}
#Para elegir únicamente algunas variables, usamos select.
cran %>%
...
```{r}
#Ahora seleccionamos todas las columnas menos las que están entre 'date' y 'size'.
cran %>%
  select(-date:-size)
```
...
```{r}
#Ahora elegimos todos los registros en los que se haya descargado el paquete 'plotly'.
cran %>%
...
```{r}
#Ahora elegimos aquellas descargas realizadas desde España (ES), con versiones de R superiores a la 3.4.0.
cran %>%
...
```{r}
#Tenemos que elegir aquellas descargas realizadas desde España (ES), o Portugal (PT).
cran %>%
...
```{r}
#Elegimos aquellas descargas cuyo tamaño supera los 100500 bytes desde un sistema operativo 'linux-gnu'.
cran %>%
  filter(r_os == "linux-gnu", size > 100500)
```
9. Elige aquellas descargas en las que la versión del sistema operativo no está vacía.
```{r}
#Elegimos aquellas descargas en las que la versión del sistema operativo no está vacía.
cran %>%
  filter(!is.na(r_os))
```
...
```{r}
#Ordenamos cran2 según los valores de country, r_version e ip_id.
cran2 %>%
...
```{r}
#Seleccionamos el subconjunto formado por las variables del cran, ip_id, package y size.
cran3 <- cran %>%
...
```{r}
#Creamos una nueva variable llamada size_mb.
cran3 %>% 
...
```{r}
#Creamos una nueva variable llamada size_gb.
cran3 %>%
...
```{r}
#Creamos una nueva variable llamada correct_size con el valor correcto.
cran3 %>%
...
```{r}
#Usamos summarise para determinar el valor medio del tamaño de las descargas realizadas en cran y almacénalo en la variable avg_bytes.
cran3 %>%
...
```{r}
#Creamos un agrupamiento del tibble cran según la variable package y almacénalo en la variable by_package.
by_package <- cran %>%
  group_by(package)
by_package
#Deshacemos el agrupamiento con la función ungroup.
ungroup(by_package)
...
```{r}
#Usamos la función summarise sobre el elemento agrupado y calcula el valor medio de la variable size.
by_package %>% 
  summarise(ValorMedio = mean(size))
```
